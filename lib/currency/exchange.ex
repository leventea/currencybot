defmodule CurrencyBot.Currency.Exchange do
  ...
  @spec convert(float(), String.t(), list(String.t()), integer()) ::
          {:ok, list({String.t(), float()})} | {:err, String.t()}
    ...
    case :ets.lookup(table, from) do
      ...
      [{^from, source_rate}] ->
        {:ok,
         ...
         # filter out invalid lookup results
         |> filter(fn x -> x != [] end)
         # unwrap from extra list
         |> map(fn [w] -> w end)
         |> map(fn {curr, val} ->
           # worth of 1 unit of target currency
           single = val / source_rate
